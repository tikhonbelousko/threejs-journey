...
void main()
{
    ...
    // // Pattern 31
    // vec2 uv1 = vec2(vUv.x, (vUv.y - 0.5) * 5.0 + 0.5);
    // vec2 uv2 = vec2((vUv.x - 0.5) * 5.0 + 0.5, vUv.y);
    ...
    // float strength = 1.0 - step(0.01, abs(distance(newUv, vec2(0.5)) - 0.25));
    // gl_FragColor = vec4(strength, strength, strength, 1.0);
    ...
    // vec2 mid = vec2(0.5);
    // float radius = sin(angle * 100.0) * 0.05 + 0.5;
    // float circle = step(0.025, abs(distance(vUv / radius, mid / radius) - 0.5));
    // float strength = 1.0 - circle;

    // gl_FragColor = vec4(strength, strength, strength, 1.0);

    // // Pattern 45
    // float angle = atan(vUv.x - 0.5, vUv.y - 0.5) / (2.0 * PI) + 0.5;
    // float sinusoid = sin(angle * PI * 2.0 * 16.0) * 0.02;
