...
material.onBeforeCompile = shader => {
  ...
  shader.vertexShader = shader.vertexShader.replace(
    ...
    `
      ...
      mat2 getRotationMatrix2D(float angle) {
        return mat2(
          cos(angle), -sin(angle),
          sin(angle), cos(angle)
        );
  ...
  shader.vertexShader = shader.vertexShader.replace(
    ...
    `
      ...
      float angle = position.y * 0.4 + uTime;
      mat2 rotationMatrix = getRotationMatrix2D(angle);
...
depthMaterial.onBeforeCompile = shader => {
  ...
  shader.vertexShader = shader.vertexShader.replace(
    ...
    `
      #include <begin_vertex>
      mat2 rotationMatrix = getRotationMatrix2D(transformed.y * 0.4 + uTime);
      transformed.xz = rotationMatrix * transformed.xz;
    `,
  )
}
